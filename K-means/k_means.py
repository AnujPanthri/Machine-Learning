...
class k_means:
    ...
    def train(self,x,iter=100,show=True,showat=100):
        loss_list=[]
        if self.mu is None:
            self.random_clusters(x)
        for i in range(iter):
        ...
        return loss_list
    def random_initializations(self,x,iter=100,num_of_models=1000):
        mu_list=[]
        loss_list=[]
        for i in range(num_of_models):
            self.random_clusters(x)
            mu_list.append(self.mu)
            self.train(x,iter=iter,show=False)
            loss_list.append(self.loss(x))
        idx=np.argmin(loss_list)
        self.mu=mu_list[idx]

    def elbow_method(self,x,iter=100,K=10):
        original_k=self.K
        ...
        for k in k_list:
            self.K=k
            self.random_clusters(x)
            self.random_initializations(x,iter=iter,num_of_models=100)
            self.train(x,iter=iter,show=False)
            loss_list.append(self.loss(x))
        plt.figure()
    ...
    def loss_graph(self,loss_list):
        ...
        plt.show()
    def show_2D_data(self,x,title=None):
        if self.mu is None:
            self.random_clusters(x)
        C=self.x_cluster_idx(x)
